...
class Simulator extends Component {
  ...
  handleSelectorUpdate(side, unitKey, delta) {
    ...
    let currentModCount = this.state.units.modCount(side, unitKey);
    this.setState((state, props) => ({
      units: state.units.setUnitCount(
        side,
        unitKey,
        currentCount + delta,
        currentModCount
  ...
  handleModSelectorUpdate(side, unitKey, delta) {
    let currentCount = this.state.units.unitCount(side, unitKey);
    let currentModCount = this.state.units.modCount(side, unitKey);
    this.setState((state, props) => ({
      units: state.units.setUnitCount(
  ...
  handleSimulateClick() {
    this.setState((state, props) => {
      let promise = workerInstance
        .simulate(state.units.units, N, state.options)
        .then((rawResults) => {
  ...
  renderHelpModal() {
    if (!this.state.showingHelpModal) return null;

    return <HelpModal onExit={() => this.showHelpModal(false)} />;
  }
  ...
  render() {
    ...
    if (this.state.simulationResults) {
      mainItem = (
        <SimulationReview
          simulation={this.state.simulationResults}
    ...
    return (
      <div className={this.getClasses()}>
        <UnitSelector
          ...
          onUpdateMod={(unitKey, delta) =>
            this.handleModSelectorUpdate(ATTACKER_SIDE, unitKey, delta)
          }
        ...
        <UnitSelector
          ...
          onUpdateMod={(unitKey, delta) =>
            this.handleModSelectorUpdate(DEFENDER_SIDE, unitKey, delta)
          }
          onClear={(side) => this.handleUnitSummaryClear(side)}
        />
